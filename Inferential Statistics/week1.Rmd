...
samp1 %>% summarise(mean = mean(area), sam_med = median(area),
                    sam_sd = sd(area), sam_iqr = IQR(area),
                    sam_min = min(area), sam_max = max(area), 
                    sam_q1 = quantile(area, 0.25),
...
samp1 %>% summarise(price_sam_mean = mean(price))
```
...
ggplot(data = sample_means50, aes(x = x_price)) + geom_histogram()
sample_means50 %>% summarise(sam_mean = mean(x_price))
```
...
sample_means150 <- ames %>% rep_sample_n(size=150, reps = 5000, replace = TRUE) %>%
  summarise(x_price = mean(price))
ggplot(data = sample_means150, aes(x = x_price)) + geom_histogram()
sample_means150 %>% summarise(sam_mean = mean(x_price))
```
...
# type your code for this Exercise here, and Run Document
sample_15 <- ames %>% sample_n(size=15)
sample_15 %>% summarise(sam_mean = mean(price))
```
...
# type your code for this Exercise here, and Run Document
sample_means15 <- ames %>% rep_sample_n(reps = 2000, size = 15, replace = TRUE) %>%
  summarise(x_price = mean(price))
ggplot(data = sample_means15, aes(x_price)) + geom_histogram()
sample_means15 %>% summarise(guess_mean = mean(x_price), se = sd(x_price))
```
...
# type your code for this Exercise here, and Run Document
sample_means150 <- ames %>% rep_sample_n(reps = 2000, size = 150, replace = TRUE) %>%
  summarise(x_price = mean(price))
ggplot(data = sample_means150, aes(x_price)) + geom_histogram()
sample_means150 %>% summarise(guess_mean = mean(x_price), se = sd(x_price))
```
