...
ggplot(data = nc, aes(x = habit, y = gained)) + geom_boxplot()
```
...
inference(y = weight, x = habit, data = nc, statistic = "mean", type = "ci", null = 0, method = "theoretical", alternative = "twosided")
```
...
inference(y = weeks, data = nc, type = "ci", statistic = "mean", null = 0, alternative = "twosided", method = "theoretical", conf_level = 0.99)
```
...
inference(y = weeks, data = nc, type = "ci", statistic = "mean", null = 0, alternative = "twosided", method = "theoretical", conf_level = 0.90)
```
...
```{r gained-mature-ht}
# type your code for the Exercise here, and Knit
inference(y = weight, x= mature, data = nc, type = "ht", statistic = "mean", null = 0, alternative = "twosided", method = "theoretical")
```
...
nc %>% group_by(mature) %>% 
  summarise(median_mage = median(mage), mean_mage = mean(mage), max_age = max(mage), min_age = min(mage))
```
